...
export class AlertsComponent implements OnInit {
  ...
  CalculRequests() {
    this.congeService.Get().subscribe(res => {
      this.conglis = res
      this.Fconglis = this.conglis.filter(item => item.directeurid == this.idUser)
      this.nbCong = this.Fconglis.length;
    
      this.permissionService.Get().subscribe(res => {
        this.perlis = res
        this.Fperlis = this.perlis.filter(item => item.iddir == this.idUser)
        this.nbPer = this.Fperlis.length;

        this.equipementService.Get().subscribe(res => {
          this.equlis = res
          this.Fequlis = this.equlis.filter(item => item.iddir == this.idUser)
          this.nbEqu = this.Fequlis.length;

          this.recrutementService.Get().subscribe(res => {
            this.reclis = res
            this.Freclis = this.reclis.filter(item => item.iddir == this.idUser)
